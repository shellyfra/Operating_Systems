...
//TODO: fix alginment to 8 byte instead of 4 and hope for good
#include <unistd.h>
#include <assert.h>
#include <cstdlib>
#include <sys/wait.h>
...
void test_malloc_split_and_merge() {
     ...
     *p1b =  4 (used), 132 + METASIZE (free), p2 = 4 (used), p3 = 4(used) */
    sfree(p1);
    ...
    assert_state(initial, expected);
    /* p1b =  4 (used), p1 = 4 (used) , 128 (free), p2 = 4 (used), p3 = 4(used) */
    p1 = malloc_byte(4);
    expected.allocated_blocks++;
    expected.allocated_bytes -= METASIZE;
    expected.free_bytes -= (4 + METASIZE);
    ...
    /* release p3 then p2. assert that p2 is merged with both adjacent blocks. */
    /* p1b =  4 (used), p1 = 4 (used) , 128 (free), p2 = 4 (used), p3 = 4(free) */
    sfree(p3);
    free_expected_block(expected, 3);
    assert_state(initial, expected);
    sfree(p2);
    /* p1b =  4 (used), p1 = 4 (used) , 136 + 2*META(free) */
    free_expected_block(expected, 3);
    merge_expected_block(expected);
    merge_expected_block(expected);
    assert_state(initial, expected);
