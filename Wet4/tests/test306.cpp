#include "aux_macro.h"
#include "../malloc_3.cpp"
#include <iostream>

void printStats()
                    {
                        std::cout << "_num_free_blocks: " << _num_free_blocks() << std::endl;
                        std::cout << "_num_free_bytes: " << _num_free_bytes() << std::endl;
                        std::cout << "_num_allocated_blocks: " << _num_allocated_blocks() << std::endl;
                        std::cout << "_num_allocated_bytes: " << _num_allocated_bytes() << std::endl;
                        std::cout << "_num_meta_data_bytes: " << _num_meta_data_bytes() << std::endl;
                        std::cout << "_size_meta_data: " << _size_meta_data() << std::endl;
                    }
                    int main()
                    {
                        PRINT_POINTER(scalloc(40,193););
printStats();
void* p0 = last_address;
PRINT_POINTER(smalloc(136););
printStats();
void* p1 = last_address;
PRINT_POINTER(scalloc(186,88););
printStats();
void* p2 = last_address;
PRINT_POINTER(smalloc(148););
printStats();
void* p3 = last_address;
PRINT_POINTER(smalloc(222););
printStats();
void* p4 = last_address;
PRINT_POINTER(srealloc(p4,12););
printStats();
void* p5 = last_address;
PRINT_POINTER(srealloc(p5,121););
printStats();
void* p6 = last_address;
PRINT_POINTER(scalloc(61,124););
printStats();
void* p7 = last_address;
PRINT_POINTER(srealloc(p0,89););
printStats();
void* p8 = last_address;
PRINT_POINTER(srealloc(p2,224););
printStats();
void* p9 = last_address;
DEBUG_PRINT(sfree(p6););
printStats();
PRINT_POINTER(smalloc(26););
printStats();
void* p10 = last_address;
PRINT_POINTER(scalloc(18,209););
printStats();
void* p11 = last_address;
PRINT_POINTER(smalloc(95););
printStats();
void* p12 = last_address;
PRINT_POINTER(scalloc(100,1););
printStats();
void* p13 = last_address;
PRINT_POINTER(smalloc(179););
printStats();
void* p14 = last_address;
PRINT_POINTER(srealloc(p10,214););
printStats();
void* p15 = last_address;
DEBUG_PRINT(sfree(p8););
printStats();
DEBUG_PRINT(sfree(p3););
printStats();
DEBUG_PRINT(sfree(p1););
printStats();
PRINT_POINTER(scalloc(32,141););
printStats();
void* p16 = last_address;
PRINT_POINTER(scalloc(143,1););
printStats();
void* p17 = last_address;
PRINT_POINTER(smalloc(103););
printStats();
void* p18 = last_address;
PRINT_POINTER(srealloc(p12,112););
printStats();
void* p19 = last_address;
DEBUG_PRINT(sfree(p18););
printStats();
DEBUG_PRINT(sfree(p14););
printStats();
PRINT_POINTER(srealloc(p17,217););
printStats();
void* p20 = last_address;
PRINT_POINTER(srealloc(p16,70););
printStats();
void* p21 = last_address;
PRINT_POINTER(srealloc(p13,51););
printStats();
void* p22 = last_address;
PRINT_POINTER(srealloc(p22,23););
printStats();
void* p23 = last_address;
PRINT_POINTER(scalloc(122,64););
printStats();
void* p24 = last_address;
PRINT_POINTER(scalloc(188,241););
printStats();
void* p25 = last_address;
PRINT_POINTER(scalloc(246,243););
printStats();
void* p26 = last_address;
PRINT_POINTER(srealloc(p24,66););
printStats();
void* p27 = last_address;
PRINT_POINTER(smalloc(88););
printStats();
void* p28 = last_address;
DEBUG_PRINT(sfree(p27););
printStats();
PRINT_POINTER(srealloc(p21,223););
printStats();
void* p29 = last_address;
PRINT_POINTER(srealloc(p23,91););
printStats();
void* p30 = last_address;
PRINT_POINTER(smalloc(131););
printStats();
void* p31 = last_address;
DEBUG_PRINT(sfree(p25););
printStats();
PRINT_POINTER(scalloc(195,186););
printStats();
void* p32 = last_address;
PRINT_POINTER(srealloc(p19,202););
printStats();
void* p33 = last_address;
PRINT_POINTER(srealloc(p32,187););
printStats();
void* p34 = last_address;
DEBUG_PRINT(sfree(p15););
printStats();
PRINT_POINTER(smalloc(173););
printStats();
void* p35 = last_address;
DEBUG_PRINT(sfree(p28););
printStats();
DEBUG_PRINT(sfree(p29););
printStats();
DEBUG_PRINT(sfree(p7););
printStats();
PRINT_POINTER(srealloc(p26,78););
printStats();
void* p36 = last_address;
PRINT_POINTER(scalloc(162,211););
printStats();
void* p37 = last_address;
PRINT_POINTER(srealloc(p34,39););
printStats();
void* p38 = last_address;
PRINT_POINTER(scalloc(227,83););
printStats();
void* p39 = last_address;
PRINT_POINTER(srealloc(p31,63););
printStats();
void* p40 = last_address;
PRINT_POINTER(srealloc(p36,59););
printStats();
void* p41 = last_address;
PRINT_POINTER(scalloc(92,230););
printStats();
void* p42 = last_address;
PRINT_POINTER(srealloc(p30,150););
printStats();
void* p43 = last_address;
DEBUG_PRINT(sfree(p38););
printStats();
PRINT_POINTER(srealloc(p43,9););
printStats();
void* p44 = last_address;
PRINT_POINTER(smalloc(166););
printStats();
void* p45 = last_address;
DEBUG_PRINT(sfree(p20););
printStats();
PRINT_POINTER(scalloc(86,22););
printStats();
void* p46 = last_address;
DEBUG_PRINT(sfree(p37););
printStats();
PRINT_POINTER(smalloc(200););
printStats();
void* p47 = last_address;
DEBUG_PRINT(sfree(p33););
printStats();
PRINT_POINTER(srealloc(p40,209););
printStats();
void* p48 = last_address;
PRINT_POINTER(srealloc(p44,151););
printStats();
void* p49 = last_address;
PRINT_POINTER(scalloc(44,217););
printStats();
void* p50 = last_address;
PRINT_POINTER(srealloc(p35,152););
printStats();
void* p51 = last_address;
PRINT_POINTER(smalloc(6););
printStats();
void* p52 = last_address;
PRINT_POINTER(smalloc(178););
printStats();
void* p53 = last_address;
PRINT_POINTER(srealloc(p11,55););
printStats();
void* p54 = last_address;
PRINT_POINTER(srealloc(p49,229););
printStats();
void* p55 = last_address;
DEBUG_PRINT(sfree(p9););
printStats();
PRINT_POINTER(smalloc(72););
printStats();
void* p56 = last_address;
PRINT_POINTER(smalloc(40););
printStats();
void* p57 = last_address;
PRINT_POINTER(srealloc(p46,35););
printStats();
void* p58 = last_address;
PRINT_POINTER(srealloc(p52,186););
printStats();
void* p59 = last_address;
PRINT_POINTER(smalloc(226););
printStats();
void* p60 = last_address;
DEBUG_PRINT(sfree(p48););
printStats();
PRINT_POINTER(smalloc(4););
printStats();
void* p61 = last_address;
PRINT_POINTER(scalloc(214,92););
printStats();
void* p62 = last_address;
DEBUG_PRINT(sfree(p57););
printStats();
PRINT_POINTER(srealloc(p47,80););
printStats();
void* p63 = last_address;
DEBUG_PRINT(sfree(p50););
printStats();
DEBUG_PRINT(sfree(p39););
printStats();
PRINT_POINTER(srealloc(p42,143););
printStats();
void* p64 = last_address;
PRINT_POINTER(srealloc(p59,195););
printStats();
void* p65 = last_address;
PRINT_POINTER(scalloc(64,135););
printStats();
void* p66 = last_address;
PRINT_POINTER(scalloc(118,146););
printStats();
void* p67 = last_address;
PRINT_POINTER(srealloc(p62,3););
printStats();
void* p68 = last_address;
PRINT_POINTER(scalloc(245,46););
printStats();
void* p69 = last_address;
DEBUG_PRINT(sfree(p53););
printStats();
PRINT_POINTER(smalloc(50););
printStats();
void* p70 = last_address;
PRINT_POINTER(smalloc(185););
printStats();
void* p71 = last_address;
PRINT_POINTER(srealloc(p69,97););
printStats();
void* p72 = last_address;
PRINT_POINTER(srealloc(p41,185););
printStats();
void* p73 = last_address;
PRINT_POINTER(smalloc(111););
printStats();
void* p74 = last_address;
PRINT_POINTER(scalloc(30,236););
printStats();
void* p75 = last_address;
PRINT_POINTER(scalloc(42,107););
printStats();
void* p76 = last_address;
PRINT_POINTER(scalloc(245,223););
printStats();
void* p77 = last_address;
DEBUG_PRINT(sfree(p65););
printStats();
PRINT_POINTER(smalloc(96););
printStats();
void* p78 = last_address;
DEBUG_PRINT(sfree(p64););
printStats();
PRINT_POINTER(smalloc(138););
printStats();
void* p79 = last_address;
DEBUG_PRINT(sfree(p61););
printStats();
DEBUG_PRINT(sfree(p60););
printStats();
PRINT_POINTER(srealloc(p79,94););
printStats();
void* p80 = last_address;
DEBUG_PRINT(sfree(p76););
printStats();
PRINT_POINTER(smalloc(177););
printStats();
void* p81 = last_address;
PRINT_POINTER(scalloc(210,60););
printStats();
void* p82 = last_address;
DEBUG_PRINT(sfree(p54););
printStats();
PRINT_POINTER(smalloc(28););
printStats();
void* p83 = last_address;
PRINT_POINTER(scalloc(177,131););
printStats();
void* p84 = last_address;
PRINT_POINTER(smalloc(137););
printStats();
void* p85 = last_address;
DEBUG_PRINT(sfree(p85););
printStats();
PRINT_POINTER(scalloc(70,180););
printStats();
void* p86 = last_address;
DEBUG_PRINT(sfree(p68););
printStats();
DEBUG_PRINT(sfree(p71););
printStats();
PRINT_POINTER(scalloc(9,40););
printStats();
void* p87 = last_address;
PRINT_POINTER(scalloc(87,34););
printStats();
void* p88 = last_address;
DEBUG_PRINT(sfree(p56););
printStats();
PRINT_POINTER(srealloc(p77,84););
printStats();
void* p89 = last_address;
PRINT_POINTER(scalloc(154,146););
printStats();
void* p90 = last_address;
PRINT_POINTER(smalloc(131););
printStats();
void* p91 = last_address;
PRINT_POINTER(srealloc(p55,89););
printStats();
void* p92 = last_address;
PRINT_POINTER(srealloc(p92,74););
printStats();
void* p93 = last_address;
PRINT_POINTER(smalloc(151););
printStats();
void* p94 = last_address;
DEBUG_PRINT(sfree(p93););
printStats();
PRINT_POINTER(scalloc(72,172););
printStats();
void* p95 = last_address;
PRINT_POINTER(srealloc(p94,118););
printStats();
void* p96 = last_address;
PRINT_POINTER(scalloc(66,3););
printStats();
void* p97 = last_address;
DEBUG_PRINT(sfree(p58););
printStats();
PRINT_POINTER(scalloc(191,222););
printStats();
void* p98 = last_address;
PRINT_POINTER(smalloc(69););
printStats();
void* p99 = last_address;
PRINT_POINTER(smalloc(242););
printStats();
void* p100 = last_address;
DEBUG_PRINT(sfree(p70););
printStats();
PRINT_POINTER(smalloc(95););
printStats();
void* p101 = last_address;
PRINT_POINTER(scalloc(189,11););
printStats();
void* p102 = last_address;
DEBUG_PRINT(sfree(p102););
printStats();
PRINT_POINTER(smalloc(113););
printStats();
void* p103 = last_address;
PRINT_POINTER(smalloc(2););
printStats();
void* p104 = last_address;
PRINT_POINTER(srealloc(p89,97););
printStats();
void* p105 = last_address;
DEBUG_PRINT(sfree(p88););
printStats();
PRINT_POINTER(scalloc(227,151););
printStats();
void* p106 = last_address;
DEBUG_PRINT(sfree(p51););
printStats();
PRINT_POINTER(scalloc(143,60););
printStats();
void* p107 = last_address;
PRINT_POINTER(scalloc(69,79););
printStats();
void* p108 = last_address;
PRINT_POINTER(scalloc(107,178););
printStats();
void* p109 = last_address;
PRINT_POINTER(smalloc(13););
printStats();
void* p110 = last_address;
PRINT_POINTER(srealloc(p96,181););
printStats();
void* p111 = last_address;
DEBUG_PRINT(sfree(p45););
printStats();
DEBUG_PRINT(sfree(p98););
printStats();
PRINT_POINTER(smalloc(119););
printStats();
void* p112 = last_address;
PRINT_POINTER(smalloc(203););
printStats();
void* p113 = last_address;
DEBUG_PRINT(sfree(p107););
printStats();
PRINT_POINTER(srealloc(p106,93););
printStats();
void* p114 = last_address;
PRINT_POINTER(smalloc(164););
printStats();
void* p115 = last_address;
DEBUG_PRINT(sfree(p78););
printStats();
PRINT_POINTER(scalloc(11,33););
printStats();
void* p116 = last_address;
DEBUG_PRINT(sfree(p110););
printStats();
PRINT_POINTER(smalloc(77););
printStats();
void* p117 = last_address;
DEBUG_PRINT(sfree(p100););
printStats();
PRINT_POINTER(srealloc(p104,184););
printStats();
void* p118 = last_address;
DEBUG_PRINT(sfree(p117););
printStats();
DEBUG_PRINT(sfree(p63););
printStats();
PRINT_POINTER(srealloc(p113,138););
printStats();
void* p119 = last_address;
PRINT_POINTER(srealloc(p83,137););
printStats();
void* p120 = last_address;
PRINT_POINTER(scalloc(212,134););
printStats();
void* p121 = last_address;
PRINT_POINTER(srealloc(p91,230););
printStats();
void* p122 = last_address;
PRINT_POINTER(scalloc(235,94););
printStats();
void* p123 = last_address;
DEBUG_PRINT(sfree(p120););
printStats();
DEBUG_PRINT(sfree(p111););
printStats();
PRINT_POINTER(srealloc(p95,210););
printStats();
void* p124 = last_address;
PRINT_POINTER(smalloc(2););
printStats();
void* p125 = last_address;
PRINT_POINTER(srealloc(p121,119););
printStats();
void* p126 = last_address;
DEBUG_PRINT(sfree(p103););
printStats();
PRINT_POINTER(smalloc(2););
printStats();
void* p127 = last_address;
DEBUG_PRINT(sfree(p114););
printStats();
DEBUG_PRINT(sfree(p116););
printStats();
PRINT_POINTER(scalloc(26,73););
printStats();
void* p128 = last_address;
DEBUG_PRINT(sfree(p123););
printStats();
PRINT_POINTER(srealloc(p82,85););
printStats();
void* p129 = last_address;
PRINT_POINTER(smalloc(142););
printStats();
void* p130 = last_address;
PRINT_POINTER(scalloc(69,158););
printStats();
void* p131 = last_address;
PRINT_POINTER(smalloc(224););
printStats();
void* p132 = last_address;
DEBUG_PRINT(sfree(p66););
printStats();
PRINT_POINTER(scalloc(50,137););
printStats();
void* p133 = last_address;
DEBUG_PRINT(sfree(p124););
printStats();
PRINT_POINTER(srealloc(p130,114););
printStats();
void* p134 = last_address;
PRINT_POINTER(smalloc(109););
printStats();
void* p135 = last_address;
PRINT_POINTER(scalloc(116,157););
printStats();
void* p136 = last_address;
PRINT_POINTER(srealloc(p86,178););
printStats();
void* p137 = last_address;
PRINT_POINTER(smalloc(31););
printStats();
void* p138 = last_address;
PRINT_POINTER(srealloc(p122,247););
printStats();
void* p139 = last_address;
DEBUG_PRINT(sfree(p139););
printStats();
PRINT_POINTER(smalloc(124););
printStats();
void* p140 = last_address;
DEBUG_PRINT(sfree(p108););
printStats();
PRINT_POINTER(smalloc(141););
printStats();
void* p141 = last_address;
DEBUG_PRINT(sfree(p134););
printStats();
PRINT_POINTER(srealloc(p84,2););
printStats();
void* p142 = last_address;
PRINT_POINTER(srealloc(p115,55););
printStats();
void* p143 = last_address;
DEBUG_PRINT(sfree(p126););
printStats();
PRINT_POINTER(srealloc(p105,118););
printStats();
void* p144 = last_address;
PRINT_POINTER(srealloc(p87,243););
printStats();
void* p145 = last_address;
DEBUG_PRINT(sfree(p128););
printStats();
DEBUG_PRINT(sfree(p75););
printStats();
PRINT_POINTER(srealloc(p118,6););
printStats();
void* p146 = last_address;
PRINT_POINTER(smalloc(60););
printStats();
void* p147 = last_address;
PRINT_POINTER(smalloc(98););
printStats();
void* p148 = last_address;
PRINT_POINTER(smalloc(92););
printStats();
void* p149 = last_address;
PRINT_POINTER(scalloc(179,26););
printStats();
void* p150 = last_address;
PRINT_POINTER(srealloc(p147,58););
printStats();
void* p151 = last_address;
PRINT_POINTER(srealloc(p138,58););
printStats();
void* p152 = last_address;
PRINT_POINTER(srealloc(p67,198););
printStats();
void* p153 = last_address;
PRINT_POINTER(scalloc(86,142););
printStats();
void* p154 = last_address;
PRINT_POINTER(smalloc(50););
printStats();
void* p155 = last_address;
PRINT_POINTER(srealloc(p81,192););
printStats();
void* p156 = last_address;
PRINT_POINTER(smalloc(154););
printStats();
void* p157 = last_address;
PRINT_POINTER(scalloc(118,26););
printStats();
void* p158 = last_address;
PRINT_POINTER(smalloc(109););
printStats();
void* p159 = last_address;
DEBUG_PRINT(sfree(p132););
printStats();
DEBUG_PRINT(sfree(p159););
printStats();
PRINT_POINTER(smalloc(63););
printStats();
void* p160 = last_address;
DEBUG_PRINT(sfree(p142););
printStats();
DEBUG_PRINT(sfree(p135););
printStats();
PRINT_POINTER(srealloc(p143,127););
printStats();
void* p161 = last_address;
PRINT_POINTER(srealloc(p119,161););
printStats();
void* p162 = last_address;
DEBUG_PRINT(sfree(p160););
printStats();
PRINT_POINTER(scalloc(17,14););
printStats();
void* p163 = last_address;
PRINT_POINTER(scalloc(33,10););
printStats();
void* p164 = last_address;
PRINT_POINTER(scalloc(66,109););
printStats();
void* p165 = last_address;
PRINT_POINTER(smalloc(176););
printStats();
void* p166 = last_address;
PRINT_POINTER(scalloc(158,138););
printStats();
void* p167 = last_address;
DEBUG_PRINT(sfree(p157););
printStats();
DEBUG_PRINT(sfree(p125););
printStats();
PRINT_POINTER(smalloc(155););
printStats();
void* p168 = last_address;
DEBUG_PRINT(sfree(p148););
printStats();
}
